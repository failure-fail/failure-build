...
/// Resolve grok binary: `FAILURE_BINARY` env (CI) or a locally built `xai-grok-pager` binary.
pub fn grok_binary() -> PathBuf {
    if let Ok(path) = std::env::var("FAILURE_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "FAILURE_BINARY does not exist: {}", p.display());
        return p;
    }
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        ...
        // without an explicit FAILURE_HOME every spawned child shares the real
        // `%USERPROFILE%\.failure` — test 1's models_cache.json (which embeds
        ...
        .env("FAILURE_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("FAILURE_XAI_API_BASE_URL", mock_url)
        ...
        .env("FAILURE_DISABLE_AUTOUPDATER", "1");
}
