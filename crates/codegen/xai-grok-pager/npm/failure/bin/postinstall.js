#!/usr/bin/env node
// Runs once after npm install/update. Reads the failure binary from the
// matching per-platform optional dependency (@failure-build/failure-<platform>)
// and installs it to ~/.failure/bin/ using versioned filenames:
//
//   Unix:    failure-<version>  +  failure  (symlink)
//   Windows: failure-<version>.exe  +  failure.exe  (copy)
//
...
if (!SUPPORTED.has(key)) {
    console.error(`@failure-build/failure: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@failure-build/failure-${key}`;
    try {
...
if (!version) {
    console.error('@failure-build/failure: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@failure-build/failure: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@failure-build/failure: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running failure processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid false positives
// against unrelated binaries sharing a common prefix.
...
if (!platformDir) {
    console.error(`@failure-build/failure: platform package @failure-build/failure-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @failure-build/failure');
    process.exit(0);
}

installBinary('failure', platformDir, `failure${EXE}`);
cleanupOldVersions('failure');

// Write installer config
const configDir = path.join(os.homedir(), '.failure');
...
// Persist the npm registry so `failure update` and the trampoline use the same one.
const npmRegistry = process.env.FAILURE_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @failure-build:registry',
...
// Set FAILURE_INSTALL_COMPLETIONS=1 to auto-generate to ~/.failure/completions.
const FAILURE_PATH = path.join(CANONICAL_DIR, `failure${EXE}`);
if (process.env.FAILURE_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.failure', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'failure.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_failure');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(FAILURE_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(FAILURE_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.failure/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: failure completions bash > ~/.local/share/bash-completion/completions/failure');
    console.log('     failure completions zsh  > ~/.zsh/completions/_failure');
}
