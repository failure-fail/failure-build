...
//   1. Brotli-compresses the built binary into `../failure-<platform>/bin/<bin>.br`
...
// (`@failure-build/failure`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw failure binary is
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `failure-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] failure-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'failure',
            envVar: 'FAILURE_DARWIN_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'release', 'xai-grok-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'failure',
            envVar: 'FAILURE_DARWIN_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-apple-darwin', 'release', 'xai-grok-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'failure',
            envVar: 'FAILURE_LINUX_X64',
        ...
        {
            platform: 'linux', arch: 'arm64', binName: 'failure',
            envVar: 'FAILURE_LINUX_ARM64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'xai-grok-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'failure.exe',
            envVar: 'FAILURE_WIN32_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'xai-grok-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'failure.exe',
            envVar: 'FAILURE_WIN32_ARM64',
