...
/// Like [`seed_fake_oauth`], but under the `FAILURE_LOCAL_AUTH` dev issuer
...
/// `FAILURE_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let grok_home = content.home().join(".failure");
    std::fs::create_dir_all(&grok_home).expect("create temp .failure");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("FAILURE_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("FAILURE_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("FAILURE_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
