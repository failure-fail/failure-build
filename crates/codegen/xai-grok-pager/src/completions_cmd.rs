//! `failure completions <shell>` — generate shell completion scripts.
//!
...
/// `$line[2]` empty, and the dispatch falls through — so `failure worktree <TAB>`
...
mod tests {
    ...
    // so `failure worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
