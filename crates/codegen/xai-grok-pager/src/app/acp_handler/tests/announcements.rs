#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.failure` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.failure` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.failure` reads).
    #[test]
