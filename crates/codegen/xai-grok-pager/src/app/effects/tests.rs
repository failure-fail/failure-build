...
/// Redirect `FAILURE_HOME` to a tempdir for test isolation.
fn setup_grok_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("FAILURE_HOME", tmp.path());
    }
    tmp
}
...
/// Neutralize `FAILURE_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(FAILURE_AGENT)]` (process-global env).
fn without_grok_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("FAILURE_AGENT", "")
}
...
#[serial_test::serial(FAILURE_AGENT)]
#[test]
