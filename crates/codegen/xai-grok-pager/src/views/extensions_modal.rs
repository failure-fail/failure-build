...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    ...
    // Plugin / installed-plugin dirs, under the user grok home (FAILURE_HOME-aware)
    // or a project-scoped `{cwd}/.failure/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User grok home (FAILURE_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(grok.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.failure/<subdir>/<name>` anywhere in the path.
    ...
    // Project hooks
    if source_dir.ends_with("/.failure/hooks") || source_dir.contains("/.failure/hooks/") {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.failure/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // grok-home branch is FAILURE_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.failure/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.failure/installed-plugins/vendor-abc123/skills");
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.failure/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
