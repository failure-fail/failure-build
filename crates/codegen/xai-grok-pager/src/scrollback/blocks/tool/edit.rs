...
mod tests {
    ...
    /// Pins FailureNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_failurenight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::failurenight().diff_uses_line_fg(),
            "FailureNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_failurenight_syntect` (lock is not reentrant).
...
"
    ...
    fn syntax_highlight_splits_keyword_and_string_fg() {
        let _guard = pin_failurenight_syntect();
    ...
    fn file_scoped_stale_theme_falls_back_to_hunk_only() {
        ...
        let stale = ThemeKind::FailureDay;
