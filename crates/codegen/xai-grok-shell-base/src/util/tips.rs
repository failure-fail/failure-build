...
//! persisted to `~/.failure/tip_cursor.json`.
...
/// Load the cursor from `~/.failure/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.failure/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.failure/tip_cursor.json`, so every session sees the next tip
