...
//!
//! Persists per-folder trust decisions to `~/.failure/trusted_folders.toml`.
...
/// Filename of the folder-trust store under `~/.failure/`.
...
/// ROOT (via the `~/.failure/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.failure/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.failure/trusted-hook-projects` file listed one canonical project
