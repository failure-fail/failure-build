...
//! [`crate::trust::TrustStore`] (`~/.failure/trusted_folders.toml`). The
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.failure`).
...
/// same `~/.failure/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .failure/lsp.json / hooks) \
