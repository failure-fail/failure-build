...
pub struct WorkspaceShared {
    ...
    /// (from `FAILURE_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$FAILURE_WORKSPACE_HOME` — the workspace-owned on-disk state root
    ...
    /// (`FAILURE_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    ...
    /// enabled (`FAILURE_WORKSPACE_TOOL_DEFS_ENABLED=true`).
