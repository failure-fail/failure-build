...
//!
//! ```text
//! <cwd>/.failure/rewind-checkpoints/
...
/// Directory (under `<cwd>/.failure`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.failure/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
