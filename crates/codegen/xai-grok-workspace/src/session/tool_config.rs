...
/// enabled (`FAILURE_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("FAILURE_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `FAILURE_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
pub struct WorkspaceSessionContextFactory {
    ...
    /// Resolved `$FAILURE_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$FAILURE_WORKSPACE_HOME`). Callers should only invoke this when
...
/// `FAILURE_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> xai_grok_tools::implementations::grok_build::web_fetch::WebFetchConfig
{
    ...
    if std::env::var("FAILURE_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("FAILURE_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("FAILURE_WEB_SEARCH_MODEL").unwrap_or_else(|_| "grok-4.20-multi-agent".to_string())
}
...
mod tests {
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "FAILURE_WORKSPACE_TOOL_STATE_ENABLED";
