...
/// Returns `~/.failure/worktrees/<repo_slug>` for the given git root.
///
...
/// Resolves the worktree base directory (`~/.failure/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.failure/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.failure/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.failure/worktrees/`.
...
///
/// When the source path is already inside `~/.failure/worktrees/<repo>/`, the
...
/// Scan `~/.failure/worktrees/<repo_name>/` for subdirectories not tracked
