...
//!
//! These tests touch global process state — `FAILURE_HOME` (a `OnceLock` in
//! `xai-grok-config`), `FAILURE_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `FAILURE_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the FAILURE_HOME isolation used in other
//! integration tests.
...
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under FAILURE_HOME,
...
//
// The function honors `FAILURE_TEST_VERSION` for testing. We exercise it
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("FAILURE_TEST_VERSION", "9.9.9");
    }
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    unsafe {
        std::env::set_var("FAILURE_TEST_VERSION", "0.0.0-test");
    }
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("FAILURE_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = xai_grok_update::version::get_installed_grok_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("FAILURE_TEST_VERSION", "not-a-version");
    }
    let v = xai_grok_update::version::get_installed_grok_version();
