...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".failure").join("downloads");
    let bin = home.join(".failure").join("bin");
...
fn assert_active_grok_runs(home: &Path) {
    let link = home.join(".failure").join("bin").join("grok");
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("FAILURE_BIN_DIR", home.join(".failure").join("bin"))
        .env("FAILURE_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
