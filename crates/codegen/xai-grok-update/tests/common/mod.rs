...
//! its own `OnceLock<FAILURE_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the FAILURE_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes FAILURE_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// FAILURE_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `FAILURE_HOME`, initialized exactly once per test
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("FAILURE_TEST_VERSION");
            std::env::remove_var("NPM_TOKEN");
            std::env::remove_var("FAILURE_INSTALLER");
            std::env::remove_var("FAILURE_MANAGED_BY_NPM");
            std::env::remove_var("FAILURE_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `FAILURE_HOME` between tests so each test sees a clean home.
...
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("FAILURE_TEST_VERSION", v) };
}
...
/// Backdate every file in `FAILURE_HOME/downloads` by ~2 hours.
///
