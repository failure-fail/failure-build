//! Parse marketplace sources from `~/.failure/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.failure/` and `~/.claude/`.
...
/// instead of `~/.failure`/`~/.claude`. Each root is checked for
