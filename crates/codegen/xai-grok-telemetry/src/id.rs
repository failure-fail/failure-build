...
//! `$FAILURE_HOME` is resolved through `xai-grok-config::grok_home`.
...
/// This function caches the result in `$FAILURE_HOME/agent_id` so subsequent calls
...
///
/// Enabled only when `FAILURE_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("FAILURE_TELEMETRY_SPECIAL_USER").as_deref(),
