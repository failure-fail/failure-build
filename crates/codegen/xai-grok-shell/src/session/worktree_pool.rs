...
//!   under `~/.failure/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.failure/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.failure/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.failure/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.failure/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.failure/`.
...
/// `CleanAll` mode. Placed under `~/.failure/worktree_pool/<instance_id>/<uuid>/`.
