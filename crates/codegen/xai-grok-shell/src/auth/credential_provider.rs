...
mod tests {
    ...
    /// Serializes tests that pin `FAILURE_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `FAILURE_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("FAILURE_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("FAILURE_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("FAILURE_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("FAILURE_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
