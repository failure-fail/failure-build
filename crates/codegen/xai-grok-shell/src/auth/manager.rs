...
impl AuthManager {
    pub fn new(grok_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        xai_grok_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "FAILURE_HOME": std::env::var("FAILURE_HOME").unwrap_or_else(|_| "(unset)".into()),
                "FAILURE_AUTH_PATH": std::env::var("FAILURE_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "FAILURE_AUTH": std::env::var("FAILURE_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // FAILURE_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("FAILURE_AUTH") {
            ...
            tracing::warn!("FAILURE_AUTH set but failed to parse as JSON, falling back to file");
        }

        // FAILURE_AUTH_PATH: custom file path (overrides default $FAILURE_HOME/auth.json).
        let path = std::env::var("FAILURE_AUTH_PATH")
            .map(PathBuf::from)
    ...
    /// (inline `FAILURE_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in FAILURE_AUTH mode).
            DiskAuthState::Ok => {
                xai_grok_telemetry::unified_log::info(
