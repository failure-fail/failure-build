...
mod tests {
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("FAILURE_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("FAILURE_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`FAILURE_AUTO_PERMISSION_MODE=0`), explicit
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("FAILURE_AUTO_PERMISSION_MODE", "0") };
        assert!(
