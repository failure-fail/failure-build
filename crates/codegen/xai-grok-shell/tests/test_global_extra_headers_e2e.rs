...
//! caller-owned `$FAILURE_HOME` whose `config.toml` sets every global `[models]`
...
async fn global_models_config_reaches_inference_request() {
    ...
    std::fs::create_dir_all(&grok_home).expect("create .failure home");
    std::fs::write(
...
"#,
    ...
    cmd.env_remove("FAILURE_LEADER_SOCKET");
