...
//! 4. Smoke-validate session JSON under `$FAILURE_HOME/sessions/` after exit.
//!
//! Requires a built `grok` binary (`FAILURE_BINARY` or cargo-built pager) for the
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grok_guard = EnvVarGuard::set("FAILURE_HOME", &grok_home);
    ...
    // grok_home() caches the first FAILURE_HOME per process; a separate test
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/FAILURE_HOME via `cmd.env` below; this global env
    ...
    cmd.env("HOME", &home);
    cmd.env("FAILURE_HOME", &grok_home);
    ...
    // Smoke: session storage under FAILURE_HOME/sessions has JSON artifacts.
